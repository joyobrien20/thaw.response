# Joy O'Brien
# Creating phyloseq object for further data analysis, Master's thesis (resequenced samples, post-defense)

# Notes from Hannah's code review
# 1) You should run a comparison on an oridnation and with permanova of your blanks and your samples
# to better understand how they're related to each other and if there's any cause to remove your samples if they look like blanks <see section of code>.

# 2) Compare your 1st sequencing run to your second sequencing run to make sure that 
...
# permanovas to do this as well <see section of code>. 


# This script does the following: 
# Removes sample blanks/cleans up sequence data (blanks are filtered)
# Compares sequenced and resequenced samples (post-defense)
# create a phyloseq object from the files exported from the DADA2 pipeline
...
dorm_reseq <- subset_samples(dorm1, original_reseq_neither == "reseq")# yay this worked! 
...
# Preparing the original sequence data 
#orig_seq_counterparts <- read_excel("~/Desktop/incubation_16S_reseq.xlsx", sheet = "orig_seq")
dorm_original <- subset_samples(dorm1, original_reseq_neither == "original")
original_otu_table <- otu_table(dorm_original)

# Transform the OTU table
original_otu_transform <- t(sqrt(original_otu_table))

# Run an NMDS 
orig_seq_dist <- vegdist(original_otu_transform, method = "bray")
orig_seq.nmds <- metaMDS(orig_seq_dist, k = 2, trymax = 1500)

# Plot NMDS # see which samples they are? 
plot(orig_seq.nmds)
plot(dorm_reseq.nmds)

str(orig_seq_dist)
str(dorm_reseq_dist)

orig_mat <- as.matrix(orig_seq_dist)
reseq_mat <- as.matrix(dorm_reseq_dist)
# PERMANOVA 
perm <- adonis2(reseq_mat ~ orig_mat, permutations = 999, method = "bray")

print(perm)
summary(perm)

# Pick up here on March 6
# Notes/workflow from Sean for comparing sequence runs
# Step 1: ASV table into distance matrix which becomes Y value in linear model
# Step 2: Add factors in from metadata
library(vegan)
library(phyloseq)
metadata <- as(sample_data(physeq_object), "data.frame")
adonis2(distance(physeq_object, method="bray") ~ factor1 + factor2,
        data = metadata)

# What was wrong with my approach? Reseq ASV is explained through original matrix, which is more of a mantel test to see 
# if the two are correlated instead of a Permanova

# Hannah's message/notes on how to approach
...
# Calculate distance matrix using bray --do rarifying first! Use your final phyloseq object for this?
# Make a new script for this
dorm.dist <- vegdist()
