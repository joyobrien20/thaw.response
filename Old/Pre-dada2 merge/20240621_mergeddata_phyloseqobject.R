...
# June 21, 2024; bringing in the final seq table (additive merging of samples)
...
seq_tab <- read_excel("~/GitHub/Masters_publication/incubation_16S_reseq.xlsx", sheet = "seqtab_final_merged") # fill this in with the file path for your DADA2 seqtab output
...
metadata <- read_excel("~/GitHub/Masters_publication/incubation_16S_reseq.xlsx", sheet = "metadata_final_merged") # fill this in with the metadata file 
...
dorm_merged <- phyloseq(ASV, TAX, samples) #dorm_rt = dorm_rarefy test

# Prune samples from the phyloseq object dorm created above, and call the pruned object dorm1
dorm_merged <- prune_samples(sample_sums(dorm_merged) > 0,dorm_merged)
dorm_merged #24172 taxa
# Remove taxa that are unassigned at the Phylum level
dorm_merged <- subset_taxa(dorm_merged, !is.na(Phylum)) # Note that sample blanks are still included here 
...
dorm_merged <- subset_taxa(dorm_merged, Family != "Mitochondria")
dorm_merged <- subset_taxa(dorm_merged, Order != "Chloroplast") 
# check to see if mitochondria and chloroplasts were removed
dorm_merged #13620 taxa
...
dorm_merged_noblank <- subset_samples(dorm_merged, sample_blank == "Sample")

# Check to see if samples were removed (should be 5 for the re-seq round)
dorm_merged_noblank # this checks out, 5 samples were removed; with merged dataset we are now working with 50 samples

# Rarifaction step

# Joy's rarefaction step

# Rarify OTU table of the new object (dorm_merged_noblank) --based on previous exploration I am rarefying at 4400
otu_rarify <- as.data.frame(dorm_merged_noblank@otu_table)
rarecurve(otu_rarify, step = 50, cex = 0.5, label = FALSE)
dorm_merged_noblank.rarefied <- rarefy_even_depth(dorm_merged_noblank, rngseed = 1, sample.size = 4400, replace = FALSE)

# Now let's check the rarifaction plot after rarifying 
rarify.check <- as.data.frame(dorm_merged_noblank.rarefied@otu_table)
rarecurve(rarify.check, step = 50, cex = 0.5, label = FALSE)

# Create the rarified OTU table
total_otu_table_dorm_merged_noblank.rarefied <- otu_table(dorm_merged_noblank.rarefied)

# Transform the OTU table 
dorm_merged_transformed_otu<- t(sqrt(total_otu_table_dorm_merged_noblank.rarefied))

# Create a distance matrix using vegan and bray
dorm_merged_vegdist <- vegdist(dorm_merged_transformed_otu, method = "bray")
# I did the following to check how many samples made it through rarifaction
dorm_merged_vegdist_mat<- as.matrix(dorm_merged_vegdist)
...
dorm_merged_vegdist.nmds <- metaMDS(dorm_merged_vegdist_mat, k = 2, trymax = 1500) ## "k" is the number of axes, sometimes 2 is not enough, "trymax" is the number of times it tries to find solutions
print(dorm_merged_vegdist.nmds)
plot(dorm_merged_vegdist.nmds)

plot_ordination(
  physeq = dorm_merged_noblank.rarefied,
  ordination = dorm_merged_vegdist.nmds,
  color = "site",
  shape = "pre_post_thaw",
...
saveRDS(dorm_merged_noblank.rarefied,"~/GitHub/Masters_publication/Masters_publication_R/dorm_merged_phyloseq.rds")
saveRDS(dorm_merged_noblank.rarefied@otu_table,"~/GitHub/Masters_publication/Masters_publication_R/dorm_merged_OTU.rds")
saveRDS(dorm_merged_noblank.rarefied@tax_table,"~/GitHub/Masters_publication/Masters_publication_R/dorm_merged_tax_table.rds" )
saveRDS(dorm_merged_noblank.rarefied@sam_data,"~/GitHub/Masters_publication/Masters_publication_R/dorm_merged_sam_data.rds")



# END of merged data phyloseq object script
