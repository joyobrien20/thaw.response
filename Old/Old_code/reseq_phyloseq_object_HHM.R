...
dorm_blanks <- subset_samples(dorm1, sample_blank == "BLANK")
...
reseq_otu_table <- otu_table(dorm_reseq)

dorm_reseq_transform <- t(sqrt(reseq_otu_table)) # 

dorm_reseq_dist <- vegdist(dorm_reseq_transform, method = "bray")
...
# okay you are so close, but what you need to do is compare the seq to their re-seq counterparts-not the entire first seq run 
# so for when you pick this back up over break: 
# subset the seq matrix below by the sample names (the unique sample names for the original ones)
# then run nmds and permanova 
# the thing is that there are going to be significant differences,, because the run was so different 
print(perm)
summary(perm)

# Run an NMDS of the reseq samples
