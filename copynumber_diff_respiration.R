...
# DIFFERENCE IN COPY NUMBER CODE! 
...
cpnumb_diff <- read_excel("~/Desktop/incubation_physical_chemical.xlsx", sheet = "copynumb_diff")
...
shapiro.test(cpnumb_diff$copy_number_diff) #p-value = 1.273e-05 Data is not normal
...
respcpnumb <- merge(resp4copy,cpnumb_diff) 

# Okay let's run a Kruskall Wallis and then a dunn test (maybe) for cum resp and pre-thaw copy numbers
kruskal.test(copy_number_diff ~ Cumulative_Respiration, data = respcpnumb) # Kruskal-Wallis chi-squared = 21, df = 21, p-value = 0.4589
kruskal.test(copy_number_diff ~ site, data = cpnumb_diff) # significant p = 0.01

kruskal.test(copy_number_per_gsoil_POST ~ site, data = cpnumb) #significant p-value = 0.03
dunn_test(copy_number_diff ~ site, data = cpnumb_diff)
...
resp_diff_corr <- cor.test(x = cpnumb_diff$copy_number_diff, y = respcpnumb$Cumulative_Respiration, method = "spearman")
print(resp_diff_corr)

# Visualize here 
ggplot(respcpnumb, aes(x = copy_number_diff, y = Cumulative_Respiration)) + 
  geom_point(aes(color = site), size = 3) +
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black") +
  theme_classic() +
  xlab("log10 Change in Copy number g-1 soil") +
