...
# Microbial stats on merged Phyloseq object
# June 21, 2024

# RE- sequenced updated script with final phyloseq object 
...
# Read in the phyloseq object "dorm_merged_phyloseq"
...
presamples <- subset_samples(dorm_merged_phyloseq, pre_post_thaw == "pre")
pre_meta <- subset_samples(dorm_merged_phyloseq@sam_data, pre_post_thaw == "pre")
...
postsamples <- subset_samples(dorm_merged_phyloseq, pre_post_thaw == "post")
post_meta <- subset_samples(dorm_merged_phyloseq@sam_data, pre_post_thaw == "post")
...
plot_richness(dorm_merged_phyloseq, x = "site", color = "pre_post_thaw", measures = c("Shannon")) +
  geom_boxplot() +
  theme_classic() +
  xlab("Site") +
...
# Run diversity analysis
shansimp <- estimate_richness(dorm_merged_phyloseq, measures = c("Simpson", "Shannon"))
print(shansimp)

shan <- estimate_richness(dorm_merged_phyloseq, measures = c("Shannon"))
# Export as a CSV
...
#**********************************************************************************
# correct code for figure and legend order
