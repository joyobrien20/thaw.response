...
# Need to go back and re-do abs abundance for all sites instead of relative abundance 
# But maybe for ESA you can use relative abundance

# Thaw response code (response ratio analysis code from Hannah Holland-Moritz, modeled and modified from the  
# following publication: Fan, K., Holland-Moritz, H., Walsh, C., Guo, X., Wang, D., Bai, Y., Zhu, Y., Fierer, N. & Chu, H. (2022). 
...
dormOTU <- dorm1rarefied_OTU_reseq %>%
  data.frame()
# Read in file (load OTU table) 
thaw_response <- absabund_allsites
...
write.csv(dormOTU, "~/Desktop/dormOTU_reseq.csv")
...
# Using relative abundance table, creates average (go through and double check these!)
# treatment_names <- c("sample1", "sample2"...)
ave1 = apply(thaw_response[,c(13:20,22:27,31:34,38,39)],1,mean,na.rm = TRUE);# Control # might have to change these for each site csv
ave2 = apply(thaw_response[,c(1:12,21,28:30,35:37,40:43)],1,mean,na.rm = TRUE)# Treatment 

# Standard Deviation of ALL samples
sd1 = apply(thaw_response[,c(13:20,22:27,31:34,38,39)],1,sd,na.rm = TRUE);# Control #
sd2 = apply(thaw_response[,c(1:12,21,28:30,35:37,40:43)],1,sd,na.rm = TRUE)# treatment # 

# How many encounters for ASV/ number of observations
num1 = apply(ToF[,c(13:20,22:27,31:34,38,39)],1,sum,na.rm = TRUE); #Control#
num2 = apply(ToF[,c(1:12,21,28:30,35:37,40:43)],1,sum,na.rm = TRUE) # Treatment #
...
# stopped here, need to figure out where res comes from/update
# goal: get the code to work and create a figure of RR for relative abundance of all sites
