...
dorm_rt <- phyloseq(ASV, TAX, samples) #dorm_rt = dorm_rarefy test

# Prune samples from the phyloseq object dorm created above, and call the pruned object dorm1
dorm_rt <- prune_samples(sample_sums(dorm_rt) > 0,dorm_rt)
dorm_rt #24172 taxa
# Remove taxa that are unassigned at the Phylum level
dorm_rt <- subset_taxa(dorm_rt, !is.na(Phylum)) # Note that sample blanks are still included here 
...
dorm_rt <- subset_taxa(dorm_rt, Family != "Mitochondria")
dorm_rt <- subset_taxa(dorm_rt, Order != "Chloroplast") 
...
dorm_rt_noblank <- subset_samples(dorm_rt, sample_blank == "Sample")

# Check to see if samples were removed (should be 5 for the re-seq round)
dorm_rt_noblank # this checks out, 5 samples were removed
...
otu_rarify <- as.data.frame(dorm_rt_noblank@otu_table)
rarecurve(otu_rarify, step = 50, cex = 0.5, label = FALSE)
dorm_rt_noblank.rarefied.reseq <- rarefy_even_depth(dorm_rt_noblank, rngseed = 1, sample.size = 1200, replace = FALSE)

# Now let's check the rarifaction plot after rarifying 
rarify.check <- as.data.frame(dorm_rt_noblank.rarefied.reseq@otu_table)
...
total_otu_table_rt_noblank_reseq <- otu_table(dorm_rt_noblank.rarefied.reseq)

# Transform the OTU table 
dorm_rt_noblank_transformed_reseq <- t(sqrt(total_otu_table_rt_noblank_reseq))

# Create a distance matrix using vegan and bray
dm_rt_noblank_reseq <- vegdist(dorm_rt_noblank_transformed_reseq, method = "bray")
# I did the following to check how many samples made it through rarifaction
dm_rt_noblank_mat_reseq <- as.matrix(dm_rt_noblank_reseq)
...
dorm_rt_noblank.nmds <- metaMDS(dm_rt_noblank_reseq, k = 2, trymax = 1500) ## "k" is the number of axes, sometimes 2 is not enough, "trymax" is the number of times it tries to find solutions
print(dorm_rt_noblank.nmds)
plot(dorm_rt_noblank.nmds)

plot_ordination(
  physeq = dorm_rt_noblank.rarefied.reseq,
  ordination = dorm_rt_noblank.nmds,
  color = "site",
  shape = "pre_post_thaw",
  ...
  geom_point(size = 2)+
  geom_text(aes(label = original_reseq_neither), size = 4, color = "black") +
  theme_classic() +
...
perm <- adonis2(dm_rt_noblank_reseq ~ dorm_rt_noblank.rarefied.reseq@sam_data[["original_reseq_neither"]] + dorm_rt_noblank.rarefied.reseq@sam_data[["site"]], permutations = 999)
summary(perm)
print(perm)
