...
crrel <- read_excel("~/Desktop/copynumbers_OTU_reseq.xlsx", sheet = "absabund_CRREL")
...
# treatment_names <- c("sample1", "sample2"...)
crrelave1 = apply(crrel[,c(4,7:9)],1,mean,na.rm = TRUE);# Control 
crrelave2 = apply(crrel[,c(1:3,5,6)],1,mean,na.rm = TRUE)# Treatment 

# Standard Deviation of ALL samples
crrelsd1 = apply(crrel[,c(4,7:9)],1,sd,na.rm = TRUE);# Control #
crrelsd2 = apply(crrel[,c(1:3,5,6)],1,sd,na.rm = TRUE)# treatment # 

# How many encounters for ASV/ number of observations
crrelnum1 = apply(ToF_crrel[,c(4,7:9)],1,sum,na.rm = TRUE); #Control#
crrelnum2 = apply(ToF_crrel[,c(1:3,5,6)],1,sum,na.rm = TRUE) # Treatment #
...
crrel_tax_response_abs <- left_join(x = crrel_R_adjust_abs, y = incubation_16S_reseq, by = c("ID" = "ASV_ID"))
