...
dorm_rt_33 <- phyloseq(ASV, TAX, samples) #dorm_rt = dorm_rarefy test

# Prune samples from the phyloseq object dorm created above, and call the pruned object dorm1
dorm_rt_33 <- prune_samples(sample_sums(dorm) > 0,dorm)
dorm_rt_33 #24172 taxa
# Remove taxa that are unassigned at the Phylum level
dorm_rt_33 <- subset_taxa(dorm_rt_33, !is.na(Phylum)) # Note that sample blanks are still included here 
...
dorm_rt_33 <- subset_taxa(dorm_rt_33, Family != "Mitochondria")
dorm_rt_33 <- subset_taxa(dorm_rt_33, Order != "Chloroplast") 
...
dorm_rt_noblank.rarefied.reseq <- rarefy_even_depth(dorm_rt_noblank, rngseed = 1, sample.size = 3300, replace = FALSE)
